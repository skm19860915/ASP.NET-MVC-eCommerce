...
(function($) {
    $.fn.PavoLiveSearch = function(opts) {
        ...
        function makeAutoSearch( wrapper ){
           ...
           $( wrapper ).find('input[name=\'search1\']').autocomplete({
                delay: 500,
                ...
                focus: function(event, ui) { 
                    if(ui.item){ 
                         $( wrapper ).find('input[name=\'search1\']').val(ui.item.label);
           ...
           $(selector).find('input[name=\'search1\']').data("autocomplete")._renderMenu = function (ul, b) {
                var g=this;
           ...
           $(selector).find('input[name=\'search1\']').data("autocomplete")._renderItem = function (ul, item) {
                var html = '';
                if(show_image){
...
$(window).ready( function(){
	...
	$(".navbar-toggle").click( function(){
		 ...
		 if($('.navbar-collapse').is(':visible')) {
			...
			var duration = 700;
			$('.navbar-collapse').toggle(effect, options, duration); 
...
$(document).ready(function(){
	...
	$("body").click(function(event) {
				var cls = $(event.target).parents('li').attr('lang');
	...
	$(".addactivecls").live('click',function() {
				var th = $(this);
				$(".addactivecls").removeClass('menuactive');
				th.addClass('menuactive');
	});
});